...
//    @Query("select new me.kzv.simpleboard.domain.img.ImgResponse(i.id, i.imgUrl) from UploadImg i join fetch Board b on treat(i as BoardImg).board = b where b.id = :boardId")
    @Query("select new me.kzv.simpleboard.module.img.ImgResponse(i.id, i.imgUrl) from UploadImg i where treat(i as BoardImg).board.id = :boardId")
    ...
    @Query("select new me.kzv.simpleboard.module.img.ImgResponse(i.id, i.imgUrl) from UploadImg i where treat(i as BoardImg).board.id = :boardId or treat(i as ReplyImg).board.id = :boardId")
